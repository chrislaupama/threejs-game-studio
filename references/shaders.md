...
- Renderer/environment prerequisite and PBR materials
- Safe `onBeforeCompile` patterns
- Gradient sky and post-processing chain
- Low-cost detail and readability techniques

Concrete material, shader, and post-processing recipes. Use with
`technical-art.md` for budgets and `rendering.md` for pipeline composition.
...
Before copying a recipe, compare it with the project's installed Three.js
version and current shader chunks/addon APIs. Adapt and typecheck rather than
assuming the version-specific snippet is permanent.
...
const environmentTarget = pmrem.fromScene(new RoomEnvironment(), 0.04); // retain for teardown
scene.environment = environmentTarget.texture; // r184: RoomEnvironment() takes no args
scene.environmentIntensity = 1.0; // r184 global multiplier over per-material envMapIntensity
pmrem.dispose();

// teardown: scene.environment = null; environmentTarget.dispose();
```
...
const VignetteShader = {
  ...
  fragmentShader: `uniform sampler2D tDiffuse; uniform float uStrength, uSize; varying vec2 vUv;
    void main(){
      ...
      float edge = 1.0 - smoothstep(uSize - 0.45, uSize, d);
      c.rgb *= mix(1.0, edge, uStrength);
      gl_FragColor = c;
    }`,
};
